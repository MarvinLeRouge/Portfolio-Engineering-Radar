...
Invocation: `npx --package=knip -- knip --reporter json`, mirroring `EslintComplexityRunner`'s `npx --package=` pattern, `cwd=target_path`. Generates an audit-owned scratch `knip.json` (tempfile, deleted in `finally`, same lifecycle as `EslintComplexityRunner`'s config) with default Vite entry points (`index.html`, `src/main.{js,ts,jsx,tsx}`) to avoid the false-positive-on-real-entry-points caveat already documented from the Phase 3 pilot. `exclude_paths` under `target_path` are written to the same scratch `knip.json` as an `"ignore"` array of `<relative>/**` globs. If none of the default entry-point candidates exist in the target, the runner does not run knip and returns an unusable fallback payload (`raw_output = {"stdout": "", "stderr": "no entry point candidate found"}`, no `issues` key), so 5.2 treats the subproject as having no data instead of scoring it as clean. More sophisticated project-type detection is out of scope for this increment. Defensive JSON-payload extraction (locate the first `{"issues"` prefix in stdout) carried over from the Phase 3 finding about `console.log` pollution in target projects.

Knip 6.x JSON shape (confirmed against real knip 6.38 output): `raw_output = {"issues": [<row>, ...]}`, one row per file. Each row carries `file` plus per-category lists. `files: [{"name"}]` means the row's file is fully unused. `exports`, `types` and `enumMembers` are lists of `{"name", "line", ...}` objects. `duplicates` is a list of groups, each group a list of `{"name", "line", ...}` objects. There is no top-level `files` or `findings` key.
...
All three feed 5.2's normalizer, which counts entries per tool result and bands the count: `raw_output["findings"]` (vulture), the per-row categories of `raw_output["issues"]` (knip, see shape above), or `raw_output["violations"]` (phpmd, filtered to `ruleset == "unusedcode"`).
...
`finding_count` = number of dead-code findings per tool (vulture `findings`; knip `issues` rows, counting one item per unused file, per unused export/type/enum member, and per duplicate-export group; phpmd `violations` filtered to `ruleset == "unusedcode"`). A knip result whose `raw_output` has no `issues` key (fallback payload) is treated as no data, not as zero findings.
