...
Run natively via the target's own locally-installed Vitest (`node_modules/.bin/vitest`), same reasoning as `PestRunner` below — Vitest measures the repo's own configured test suite, not an audit-owned tool version. Empirically verified during planning: the ephemeral `npx --package=vitest --package=@vitest/coverage-v8` pattern fails in two different ways (unpinned versions hit an `ERESOLVE` peer-dependency conflict; pinned matching versions install cleanly but Vitest's coverage-provider module resolution then fails at runtime with `Cannot find package '@vitest/coverage-v8'`, even though both packages are present in the same npx cache). The local-`devDependencies` pattern does not have this problem, since npx resolves the locally installed binary and its coverage provider together.
Invocation: `node_modules/.bin/vitest run --reporter=json --outputFile=<tmp>/vitest-report.json [--coverage --coverage.reporter=json-summary --coverage.reportsDirectory=<tmp>/coverage]` (coverage flags added only when `node_modules/@vitest/coverage-v8` is also present locally; otherwise pass/fail is still reported, `coverage_percent` is simply absent). If `node_modules/.bin/vitest` does not exist, the runner returns `exit_code=127` with empty `raw_output`, same convention as `PintRunner`. The JSON report gives per-test pass/fail counts directly (already validated in `toolchain.md`: 419/419 on GeoChallenge-Tracker); `<tmp>/coverage/coverage-summary.json`'s `total.lines.pct` gives the coverage percentage when present, read fresh the same way as pytest above (never a committed `coverage-summary.json`).
...
- Every `npx` invocation pins its package explicitly (`--package=<exact-name> --`), never a bare binary name, per the dependency-confusion rule in `toolchain.md`. This does not apply to `VitestRunner`, which invokes the target's own locally-installed binary directly (`node_modules/.bin/vitest`), same exception already established for `PintRunner`/`PestRunner`'s `vendor/bin/*` invocations.
